...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  if (password.length < 8 || password.length > 16) {
    return next(
      ErrorHandler("Password must be between 8 and 16 characters.", 400)
    );
  }
  ...
  if (isAlreadyRegistered.rows.length > 0) {
    return next(
      ErrorHandler("User already registered with this email.", 400)
    );
  }
...
export const login = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(ErrorHandler("Please provide email and password.", 400));
  }
...
export const forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    return next(ErrorHandler("Email could not be sent.", 500));
  }
});
...
export const resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (user.rows.length === 0) {
    return next(ErrorHandler("Invalid or expired reset token.", 400));
  }
  if (req.body.password !== req.body.confirmPassword) {
    return next(ErrorHandler("Passwords do not match.", 400));
  }
  if (
...
export const updatePassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!isPasswordMatch) {
    return next(ErrorHandler("Current password is incorrect.", 401));
  }
  if (newPassword !== confirmNewPassword) {
    return next(ErrorHandler("New passwords do not match.", 400));
  }
...
export const updateProfile = catchAsyncErrors(async (req, res, next) => {
  ...
  if (name.trim().length === 0 || email.trim().length === 0) {
    return next(ErrorHandler("Name and email cannot be empty.", 400));
  }
  let avatarData = {};
